...
class Forms(Form):
    ...
    authorFirstNew = StringField('Author First Name', [validators.Length(min=1,max=40)])
    authorLastNew = StringField('Author Last NAme', [validators.Length(min=1,max=40)])
    editionNew = StringField('Edition', [validators.Length(min=1,max=20)])
    isbnNew = StringField('ISBN', [validators.Length(min=1,max=30)])
    publisher_id = StringField('Publisher', [validators.Length(min=1,max=60)])
