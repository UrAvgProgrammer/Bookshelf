...
def home(page_num):
    ...
    return render_template('homepage.html', page_num=page_num, top=top, current_user=current_user, form=form, most=most,
                           ...
                           mostauth=mostauth, mostqnt=mostqnt, most1=most1, mostauth1=mostauth1, mostqnt1=mostqnt)
...
def ratencomm(user_id):
    ...
    if user_id == current_user.id:
        ...
        return render_template('commNrating.html', form=form, rating=rating, rates=rates, x=x, count=count)
    else:
        user = User.query.filter_by(id=user_id).first()
        otheruserId = user_id
        rating = UserRateTotal.query.filter_by(userRatee=user_id).first()
